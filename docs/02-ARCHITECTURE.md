...
This document records how I designed the system, why each architectural boundary exists, and how data moves through the project at runtime.
...
**Nginx Reverse Proxy**
- Purpose: I use Nginx as the single HTTP entry point so the browser never has to know the internal container topology.
- Responsibilities: It routes API traffic to FastAPI, serves the compiled React application, handles production SSL termination, compresses responses, and applies static-asset caching.
- Interfaces: It exposes port 80 and optionally 443, then proxies backend traffic to the internal FastAPI service on port 8000.

**FastAPI Backend**
- Purpose: I use FastAPI as the backend boundary for authentication, scan orchestration, result retrieval, and request validation.
- Responsibilities: It validates incoming schemas, enforces JWT-based authentication, coordinates scanner execution, persists scan output, and returns normalized JSON responses.
- Interfaces: It exposes `/auth/*` and `/scans/*` route groups and connects to PostgreSQL through SQLAlchemy sessions.

**React Frontend**
- Purpose: I use the React frontend as the browser-facing control plane for creating scans and reviewing stored findings.
- Responsibilities: It validates form state, calls the backend API, manages local and server state, and renders scan results in a readable interface.
- Interfaces: It communicates through the `/api` prefix behind Nginx and renders entirely in the client browser.

**PostgreSQL Database**
- Purpose: I use PostgreSQL as the durable store for accounts, scan metadata, and individual vulnerability findings.
- Responsibilities: It enforces relational integrity, stores structured scan history, indexes common lookup paths, and keeps scanner evidence attached to the correct scan record.
- Interfaces: It accepts backend connections on port 5432 inside the Docker network and enforces foreign-key relationships between users, scans, and results.

**Scanner Modules**
- Purpose: I isolate each vulnerability test in a scanner module so detection logic remains separate from routing, persistence, and UI concerns.
- Responsibilities: Each scanner sends controlled HTTP requests, evaluates responses, determines vulnerability status, and captures evidence for storage.
- Interfaces: Scanners inherit shared request behavior from `BaseScanner` and return `TestResultCreate` schemas for the service layer to persist.

## Data Flow

### Primary Runtime Path: Creating and Running a Scan

This path shows the actual request lifecycle when a scan is created. I keep the sequence explicit because it documents where validation, authorization, scanner orchestration, and persistence occur:
...
Representative implementation path with code references:
...
### Secondary Runtime Path: User Registration Flow
...
I use repositories as the boundary between business logic and SQLAlchemy queries. Database access is centralized in repository classes instead of being scattered across route handlers or services.
...
**Why I chose it:**
This keeps service methods focused on application behavior instead of query construction. A service can call `UserRepository.get_by_email(db, email)` without knowing the SQLAlchemy expression behind it. If I later replace SQLAlchemy or restructure persistence, the service layer can stay stable while the repository implementation changes.

**Trade-offs:**
- Pros: I can mock repositories in service tests, keep queries in one place, and preserve the option to swap persistence details later.
- Cons: The abstraction creates more files and navigation overhead than direct model queries would.
...
The service layer depends on the repository boundary like this:
...
**Role in this codebase:**
I use FastAPI dependencies to make route prerequisites explicit. Database sessions, authentication state, and rate-limiting context are injected into route handlers instead of being manually constructed inside each handler.

**Where I use it:**
Route handlers in `routes/auth.py` and `routes/scans.py` rely on these dependencies to keep HTTP handling thin:
...
**Why I chose it:**
The route handler only describes what it needs. It does not manually open database sessions, decode JWTs, or parse request bodies. `get_db` owns session lifecycle, `get_current_user` owns authentication, and Pydantic owns request validation.

This keeps endpoint functions small and makes isolated testing easier because I can override dependencies instead of recreating the full request cycle.

**Trade-offs:**
- Pros: I get reusable dependency boundaries, explicit handler requirements, and automatic cleanup for database sessions.
- Cons: Failures can be less obvious because dependency resolution happens before route logic runs.
...
**Role in this codebase:**
I use `BaseScanner` as a template for scanner behavior. The shared class owns request pacing, retry behavior, session setup, and timing utilities, while each concrete scanner owns its vulnerability-specific detection logic.

**Where I use it:**
`scanners/base_scanner.py` centralizes the common scanner machinery:
...
Concrete scanners then implement the vulnerability-specific `scan()` flow:
...
**Why I chose it:**
This avoids duplicating request spacing, retry logic, session management, and timing helpers across every scanner. The cost is tighter coupling to the base class, but the trade-off is acceptable here because every scanner needs the same outbound HTTP safety controls.

**Trade-offs:**
- Pros: I get consistent request behavior, less duplicated scanner code, and a predictable extension point for new scanners.
- Cons: Inheritance can become limiting if future scanners need radically different execution models.

## Layer Separation

I split the backend into three layers so HTTP handling, business orchestration, and persistence do not collapse into the same files:
...
The separation lets me test each part of the backend at the correct level:
- Routes can be tested with mocked services.
- Services can be tested with mocked repositories.
- Repositories can be tested against a real test database.

It also gives each layer a narrow responsibility. Routes handle HTTP concerns, services coordinate application behavior, and repositories perform database access.
...
- Boundary rule: Routes should not perform direct database access, implement business logic, or call repositories directly.

Representative route boundary:
...
- Boundary rule: Services should not depend on HTTP request/response objects or write raw SQL queries directly.

Representative service orchestration method:
...
- Boundary rule: Repositories should not implement business rules, handle HTTP concerns, or coordinate unrelated repositories.

Representative repository method:
...
**Computed properties** (derived at runtime, not stored):
...
The JSON columns let each scanner persist evidence that matches its detection method without forcing a schema migration for every new scanner-specific field.

## Security Architecture

### Threat Model

The main risks I designed against:
...
4. **SQL injection in scanner payloads** - Malicious user creates scan with SQLi payload as target URL hoping to exploit my database. Defense: All database access uses parameterized queries via SQLAlchemy, never concatenation.
...
The risks I explicitly keep out of scope:
...
**Why I use multiple layers:**
I do not rely on one security check to protect the whole system. If an edge rate limit is bypassed, application-level limits still apply. If a request reaches a route with a valid token, the service layer still checks resource ownership before returning scan data.

### Authentication Flow

The JWT authentication cycle is implemented as a sequence of explicit boundaries:
...
I apply rate limiting at multiple layers because inbound API abuse and outbound scanner behavior are separate problems:

**1. Nginx level** - I leave this out of the development path, but production Nginx can enforce edge-style request limits with `limit_req`:
```nginx
limit_req_zone $binary_remote_addr zone=api_limit:10m rate=100r/m;
limit_req zone=api_limit burst=20 nodelay;
```

**2. Application level** - I use SlowAPI for per-endpoint limits inside FastAPI (`backend/factory.py:34-36`):
```python
...
Those limits are applied directly to route handlers:
```python
...
**3. Scanner level** - I pace outbound requests to target APIs in `BaseScanner` (`base_scanner.py:64-90`):
```python
...
This protects target APIs from uncontrolled request bursts and makes scanner timing behavior more predictable.
...
- MongoDB: Better for schema-less data, but this project has clear relationships and benefits from foreign-key constraints.
- SQLite: Simpler setup, but less appropriate for concurrent scan writes and multi-user history.
...
I rely on SQLAlchemy connection pooling because opening a database connection for every request would be wasteful:
...
The default pool behavior supports a small local deployment without extra tuning. The request-scoped session lifecycle is managed by dependency injection:
...
I centralize runtime configuration in `.env` and `config.py`:
...
Development reads from a `.env` file through Docker Compose. Production should inject environment variables through the deployment platform, container runtime, or orchestration layer.

Settings that must be reviewed before any production-style deployment:
...
The main performance constraints come from scanner runtime, relationship loading, and intentionally expensive password hashing:
...
The main optimizations I included are:
...
Changes required for a larger deployment:
...
**What I chose:**
I use the synchronous `requests` library for scanner traffic instead of async `httpx` or `aiohttp`.
...
Costs of the synchronous approach:
...
I prioritized timing accuracy over raw scan speed. Time-based blind SQL injection detection (`sqli_scanner.py:183-251`) depends on:
...
Async concurrency would introduce extra timing noise. A delay of 5.1s versus 5.3s could be network jitter rather than SQL injection behavior. Sequential requests with controlled spacing produce cleaner timing signals.

### Decision 2: Repository pattern over Active Record

**What I chose:**
I use the repository pattern, such as `UserRepository.get_by_email(db, email)`, instead of Active Record-style model methods such as `User.find_by_email(email)`.
...
**Trade-offs:**
Benefits of the repository approach:
...
Costs of the repository approach:
- More files, more navigation
- Extra abstraction layer
- Can feel like overkill for simple CRUD

**Why I made this choice:**
I chose this for testability and maintainability. Services like `AuthService.login_user()` can call `UserRepository.get_by_email()`, and tests can replace repository behavior without touching the database.

If I later migrate away from SQLAlchemy, the repository implementations change while service behavior can remain the same.

### Decision 3: JWT without refresh tokens

**What I chose:**
I use a single 24-hour JWT and do not implement a refresh-token mechanism in this version.
...
**Why I made this choice:**
I chose this to keep the authentication model small and inspectable. Adding refresh tokens would require:
- Refresh token storage (database or Redis)
- Refresh endpoint with rotation logic
- Token revocation tracking
- More complex frontend token management

For a production-grade authentication system, I would add refresh-token rotation and revocation. For this repository, the simpler design keeps the JWT flow easier to inspect.

### Decision 4: Docker Compose for local development

**What I chose:**
I run the full local stack in Docker containers through `dev.compose.yml` instead of requiring separate host-level service installs.
...
Costs of Docker Compose:
- Slower file system on Mac (volume mounts)
- Extra resource usage (containers overhead)
- Additional learning curve when debugging Docker networking, volumes, or container logs.

**Why I made this choice:**
This keeps the development environment reproducible. The stack uses the same Postgres, Python, and Node versions regardless of host OS, and the application starts through Docker Compose instead of relying on local service setup.
...
The production deployment model uses optimized containers with Nginx in front of the FastAPI backend:
...
**Infrastructure:**
A minimal deployment can run on:
- Single VPS (DigitalOcean Droplet, AWS EC2)
- Docker Compose orchestration
- SSL via Let's Encrypt (certbot) or Cloudflare proxy

Scaling beyond a single server would move these responsibilities outward:
...
1. **Validation errors** (400) - I rely on Pydantic to reject malformed request data before it reaches service logic:
```python
...
2. **Authentication errors** (401) - Invalid, expired, or malformed JWTs fail at the dependency layer:
```python
...
3. **Authorization errors** (403) - A valid user still cannot access resources owned by another user:
```python
...
4. **Not found errors** (404) - Missing records are handled as explicit resource lookup failures:
```python
...
5. **Scanner errors** - Scanner failures are captured as result records instead of crashing the whole scan:
```python
...
A new scanner integrates through the same boundaries as the existing scanner set. The extension path is:
...
The existing architecture keeps these areas stable for a new scanner:
...
The current architecture has the following intentional limitations:
...
These limitations are not accidental bugs. They are trade-offs I accepted to keep the first version understandable, bounded, and easy to reason about.
...
- Burp is a proxy-first testing platform; this project is a standalone scanner.
- Burp uses a desktop GUI; this project exposes a web interface.
- Burp is comprehensive; this project focuses on four core vulnerability classes.

Why I made different choices:
I designed this repository to expose the scanner internals rather than hide them behind a mature commercial interface. The smaller scope makes the request generation, response analysis, and persistence path easier to inspect.

### OWASP ZAP

How this project differs:
- ZAP supports passive and active scanning; this project is active-scan focused.
- ZAP can crawl and discover endpoints; this project scans explicitly provided targets.
- ZAP has broad CI/CD integrations; this project keeps integration scope smaller.

Why I made different choices:
I kept the design smaller than ZAP so the core scanner mechanics remain visible: target selection, payload generation, response comparison, result normalization, and storage.
...
This architecture document is meant to support deeper review of the implementation rather than act as a tutorial path. The companion implementation document can be used to inspect scanner behavior, authentication flow, and layer interactions in more detail. Future changes should preserve the same boundaries: routes handle HTTP, services orchestrate behavior, repositories isolate persistence, and scanners contain detection logic.
